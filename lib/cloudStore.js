...
const DEFAULT_KEY = process.env.GITHUB_TOKEN || '';
...
async function fetchCloudData(force = false) {
  if (!GITHUB_TOKEN || GITHUB_TOKEN.length < 10) {
    ...
    return { sha: null, data: { bookings: [], settings: {}, services: [], locations: [], portfolio: [] } };
  }
  const now = Date.now();
  if (!force && cloudCache && (now - lastFetchTime < CACHE_TTL_MS)) {
    return cloudCache;
  }
  ...
  try {
    ...
    if (json && json.content) {
      ...
      if (cloudCache) return cloudCache;
      return { sha: json ? json.sha : null, data: { bookings: [], settings: {}, services: [], locations: [], portfolio: [] } };
    }
    let parsed = {};
    try {
...
async function saveCloudPayload(payload, sha) {
  if (!GITHUB_TOKEN || GITHUB_TOKEN.length < 10) {
    ...
    cloudCache = { sha: null, data: fullData };
    try {
      const localFile = path.join(__dirname, '..', 'data', 'cloud_bookings.json');
      fs.writeFileSync(localFile, JSON.stringify(fullData, null, 2), 'utf8');
    } catch (e) {}
    return true;
  }
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (!currentSha || attempt > 0) {
        // Fetch latest fresh SHA directly from GitHub
      ...
      // Write to local json file if accessible
      try {
        const localFile = path.join(__dirname, '..', 'data', 'cloud_bookings.json');
        fs.writeFileSync(localFile, JSON.stringify(fullData, null, 2), 'utf8');
      } catch (e) {}
      ...
      if (res.ok) {
        ...
        if (resJson && resJson.content && resJson.content.sha) {
          cloudCache.sha = resJson.content.sha;
        }
        return true;
      } else if (res.status === 409) {
        // Conflict - retry with fresh SHA
        console.warn('saveCloudPayload: SHA conflict (409), retrying attempt', attempt + 1);
        await new Promise(r => setTimeout(r, 400));
        continue;
      } else {
        const errText = await res.text();
        console.error('saveCloudPayload error response:', res.status, errText);
      }
    } catch (err) {
      console.error('saveCloudPayload exception:', err.message);
    }
  }
  return false;
}
