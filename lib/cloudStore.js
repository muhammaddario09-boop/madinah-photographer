...
async function recordPortfolioToCloud(db) {
  const { sha, data } = await fetchCloudData();
  const portfolio = db.prepare(`SELECT * FROM portfolio ORDER BY sort_order, id DESC`).all();
...
async function syncCloudBookingsToDb(db) {
  ...
  if (data.portfolio && data.portfolio.length > 0) {
    const checkPort = db.prepare(`SELECT COUNT(*) n FROM portfolio`).get().n;
    if (checkPort === 0) {
      ...
      for (const p of data.portfolio) {
        insPort.run(p.image_url, p.title || '', p.category || 'Portrait', p.description || '', p.location || 'Madinah', p.featured ? 1 : 0, p.sort_order || 0);
      }
    }
  }
}
