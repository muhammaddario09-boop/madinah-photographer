...
function extractDriveUrl(b) {
  if (!b) return null;
  if (b.drive_url) return b.drive_url;
  if (b.special_request && typeof b.special_request === 'string' && b.special_request.includes('[DRIVE_URL]:')) {
    const match = b.special_request.match(/\[DRIVE_URL\]:\s*([^\s\n\r]+)/);
    if (match && match[1]) return match[1];
  }
  return null;
}

function cleanSpecialRequest(text) {
  if (!text || typeof text !== 'string') return '';
  return text.replace(/\[DRIVE_URL\]:\s*[^\s\n\r]+/g, '').trim();
}

/**
 * Update Google Drive URL for a booking in Supabase
 */
async function updateBookingDriveUrlInSupabase(idOrCode, driveUrl) {
  if (!isSupabaseConfigured()) return;
  try {
    const isCode = String(idOrCode).startsWith('MDN-');
    const query = isCode ? `?booking_code=eq.${idOrCode}` : `?id=eq.${idOrCode}`;
    const cleanUrl = String(driveUrl || '').trim();

    // 1. Fetch current booking to preserve special_request
    const rows = await supabaseFetch('bookings', { query: `${query}&select=id,special_request` });
    if (Array.isArray(rows) && rows[0]) {
      let reqText = rows[0].special_request || '';
      reqText = reqText.replace(/\[DRIVE_URL\]:\s*[^\s\n\r]+/g, '').trim();
      if (cleanUrl) {
        reqText = reqText ? `${reqText} [DRIVE_URL]: ${cleanUrl}` : `[DRIVE_URL]: ${cleanUrl}`;
      }
      await supabaseFetch('bookings', {
        method: 'PATCH',
        query,
        body: {
          special_request: reqText || null,
          updated_at: new Date().toISOString()
        }
      });
      console.log(`[Supabase Drive Sync] Successfully updated drive_url for ${idOrCode} in Supabase!`);
    }
  } catch (e) {
    console.error('updateBookingDriveUrlInSupabase error:', e.message);
  }
}
...
async function fetchBookingsFromSupabase(filters = {}) {
  ...
  try {
    ...
    return rows.map(b => ({
      ...
      drive_url: extractDriveUrl(b),
      special_request: cleanSpecialRequest(b.special_request),
...
/**
 * Fetch full booking details (client, payments, history) from Supabase by ID
 */
...
/**
 * Fetch booking details directly from Supabase by Booking Code (MDN-YYYY-XXXX)
 */
async function fetchBookingByCodeFromSupabase(code) {
  if (!isSupabaseConfigured() || !code) return null;
  try {
    const cleanCode = String(code).trim().toUpperCase();
    const rows = await supabaseFetch('bookings', {
      query: `?booking_code=eq.${encodeURIComponent(cleanCode)}&select=*,clients(*),services(*),packages(*),locations(*),photographers(*),payments(*)`
    });
    ...
    return {
      ...
      service: b.services || { name: 'Madinah Photoshoot' },
      package: b.packages || { name: 'Signature', duration_minutes: 60 },
      location: b.locations || { name: 'Masjid Nabawi Area' },
      photographer: b.photographers || { name: 'UMROH LENS' },
      payments: b.payments || []
    };
  } catch (err) {
    console.error('fetchBookingByCodeFromSupabase error:', err.message);
    return null;
  }
}
...
async function fetchServicesFromSupabase() {
  if (!isSupabaseConfigured()) return null;
  try {
    const services = await supabaseFetch('services', { query: '?active=eq.1&order=sort_order.asc,id.asc' });
    if (!Array.isArray(services) || services.length === 0) return null;
    const packages = await supabaseFetch('packages', { query: '?active=eq.1&order=price.asc,id.asc' }) || [];
    return services.map(s => {
      const isLong = (s.slug || '').includes('family') || (s.slug || '').includes('tour');
      const standardDur = isLong ? 120 : 90;
      let matchedPkgs = Array.isArray(packages) ? packages.filter(p => p.service_id === s.id) : [];
      if (!matchedPkgs || matchedPkgs.length === 0) {
        matchedPkgs = [{
          id: s.id * 10,
          service_id: s.id,
        ...
        matchedPkgs = matchedPkgs.slice(0, 1).map(p => ({
          ...p,
          name: 'Standard Bespoke Session',
          duration_minutes: standardDur,
          edited_photos: 40,
    ...
    console.error('fetchServicesFromSupabase error:', err.message);
    return null;
  }
}

async function updateServiceInSupabase(id, data) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('services', {
      method: 'PATCH',
      query: `?id=eq.${id}`,
      body: data
    });
    ...
    console.error('updateServiceInSupabase error:', err.message);
    return false;
  }
}

async function createServiceInSupabase(data) {
  if (!isSupabaseConfigured()) return null;
  try {
    const res = await supabaseFetch('services', {
      method: 'POST',
      body: data,
      prefer: 'return=representation'
    });
    ...
    console.error('createServiceInSupabase error:', err.message);
    return null;
  }
}

async function deleteServiceInSupabase(id) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('packages', { method: 'DELETE', query: `?service_id=eq.${id}` }).catch(() => {});
    await supabaseFetch('services', { method: 'DELETE', query: `?id=eq.${id}` });
    return true;
  } catch (err) {
    console.error('deleteServiceInSupabase error:', err.message);
    return false;
  }
}

// --- PACKAGES ---
async function updatePackageInSupabase(id, data) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('packages', {
      method: 'PATCH',
      query: `?id=eq.${id}`,
      body: data
    });
    ...
    console.error('updatePackageInSupabase error:', err.message);
    return false;
  }
}

async function createPackageInSupabase(data) {
  if (!isSupabaseConfigured()) return null;
  try {
    const res = await supabaseFetch('packages', {
      method: 'POST',
      body: data,
      prefer: 'return=representation'
    });
    ...
    console.error('createPackageInSupabase error:', err.message);
    return null;
  }
}

async function deletePackageInSupabase(id) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('packages', { method: 'DELETE', query: `?id=eq.${id}` });
    return true;
  } catch (err) {
    console.error('deletePackageInSupabase error:', err.message);
    return false;
  }
}
...
async function fetchLocationsFromSupabase() {
  if (!isSupabaseConfigured()) return null;
  try {
    const rows = await supabaseFetch('locations', { query: '?active=eq.1&order=id.asc' });
    return (Array.isArray(rows) && rows.length) ? rows : null;
  } catch (err) {
    console.error('fetchLocationsFromSupabase error:', err.message);
    return null;
  }
}

async function updateLocationInSupabase(id, data) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('locations', {
      method: 'PATCH',
      query: `?id=eq.${id}`,
      body: data
    });
    ...
    console.error('updateLocationInSupabase error:', err.message);
    return false;
  }
}

async function createLocationInSupabase(data) {
  if (!isSupabaseConfigured()) return null;
  try {
    const res = await supabaseFetch('locations', {
      method: 'POST',
      body: data,
      prefer: 'return=representation'
    });
    ...
    console.error('createLocationInSupabase error:', err.message);
    return null;
  }
}

async function deleteLocationInSupabase(id) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('locations', { method: 'DELETE', query: `?id=eq.${id}` });
    return true;
  } catch (err) {
    console.error('deleteLocationInSupabase error:', err.message);
    return false;
  }
}
...
async function fetchSettingsFromSupabase() {
  if (!isSupabaseConfigured()) return null;
  try {
    const rows = await supabaseFetch('settings', { query: '?select=key,value' });
    if (!Array.isArray(rows) || rows.length === 0) return null;
    const map = {};
    rows.forEach(r => { map[r.key] = r.value; });
    return map;
  } catch (err) {
    console.error('fetchSettingsFromSupabase error:', err.message);
    return null;
  }
}

async function updateSettingsInSupabase(settingsObj) {
  if (!isSupabaseConfigured()) return false;
  try {
    for (const [key, val] of Object.entries(settingsObj)) {
      await supabaseFetch('settings', {
        method: 'POST',
        body: { key, value: String(val) },
        prefer: 'resolution=merge-duplicates'
      });
    }
    return true;
  } catch (err) {
    console.error('updateSettingsInSupabase error:', err.message);
    return false;
  }
}

// --- PHOTOGRAPHER PROFILE ---
async function fetchPhotographersFromSupabase() {
  if (!isSupabaseConfigured()) return null;
  try {
    const rows = await supabaseFetch('photographers', { query: '?active=eq.1&order=id.asc' });
    return (Array.isArray(rows) && rows.length) ? rows : null;
  } catch (err) {
    console.error('fetchPhotographersFromSupabase error:', err.message);
    return null;
  }
}

async function updatePhotographerInSupabase(data) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('photographers', {
      method: 'POST',
      body: { id: 1, name: data.name || 'UMROH LENS', bio: data.bio || '', avatar_url: data.avatar_url || '/img/photographer-1.jpg', active: 1 },
      prefer: 'resolution=merge-duplicates'
    });
    return true;
  } catch (err) {
    console.error('updatePhotographerInSupabase error:', err.message);
    return false;
  }
}

// --- USER PASSWORD ---
async function updateUserPasswordInSupabase(email, newPasswordHash) {
  if (!isSupabaseConfigured()) return false;
  try {
    await supabaseFetch('users', {
      method: 'PATCH',
    ...
    console.error('updateUserPasswordInSupabase error:', err.message);
    return false;
  }
}
